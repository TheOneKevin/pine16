...
#define OP_RAW      (1 << 15)
#define OP_NOWRITE  (1 << 14)
#define OP_RMASK(x) (((x) & 0b11) << 12)
#define OP_ALUIMM   (1 << 11)
#define OP_AMASK(x) (((x) & 0b1111) << 7)
#define OP_WMASK(x) (((x) & 0b11) << 5)
#define OP_END      (1 << 4)

static inline int get_write_id(char* id) {
    if(id == NULL) return 0;
...
static inline int get_read_id(char* id) {
    if(id == NULL) return 0;
